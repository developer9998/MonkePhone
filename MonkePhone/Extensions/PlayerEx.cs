...
public static class PlayerEx
{
    public static string GetName(this NetPlayer player, VRRig vrRig = null, bool includePhone = true)
    {
        ...
        {
            Player punPlayer = PhotonNetwork.CurrentRoom.GetPlayer(actor);
    ...
    public static string GetName(this Player player, VRRig vrRig = null, bool includePhone = true)
    {
        ...
        string wornCosmetics = cosmeticSet != null && cosmeticSet.items != null
                                       ? cosmeticSet.items
                                                    .Where(item => !item.isNullItem && item.itemName !=
                                                                   CosmeticsController.instance.nullItem.itemName)
        ...
        if (wornCosmetics != "")
        {
            IEnumerable<CosmeticEmoji> usedCosmeticEmoji =
                    PhoneManager.Instance.Data.cosmeticEmoji.Where(emoji => wornCosmetics.Contains(emoji.cosmeticId));

            foreach (CosmeticEmoji emoji in usedCosmeticEmoji)
