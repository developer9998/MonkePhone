...
public class NetworkHandler : MonoBehaviourPunCallbacks
{
    ...
    public override void OnPlayerPropertiesUpdate(Player targetPlayer, Hashtable changedProps)
    {
        ...
        if (netPlayer.IsLocal || !VRRigCache.Instance.TryGetVrrig(netPlayer, out RigContainer playerRig) ||
            !playerRig.TryGetComponent(out NetworkedPlayer networkedPlayer))
            return;

        if (changedProps.TryGetValue(Constants.CustomProperty, out object props_object) &&
            props_object is Dictionary<string, object> properties)
        {
            networkedPlayer.HasMonkePhone = true;

            Logging.Info(
                    $"Recieved properties from {netPlayer.NickName}: {string.Join(", ", properties.Select(prop => $"[{prop.Key}: {prop.Value}]"))}");
