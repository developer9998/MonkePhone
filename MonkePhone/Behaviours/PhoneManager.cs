...
public class PhoneManager : MonoBehaviour
{
    ...
    public async void Awake()
    {
        ...
        try
        {
            ...
            CreateSound("Key",
                    "561678__mattruthsound__keyboard-computer-mechanical-typing-press-button-click-tap-three-key-presses-rapidly_96khz_mono_zoomh4n_nt5");
        }
        catch (Exception ex)
        {
        ...
        try
        {
            Phone.transform.Find("Model").GetComponent<MeshRenderer>().material.color =
    ...
    */
    ...
    public async Task Initialize()
    {
        ...
        UnityWebRequest versionWebRequest =
                UnityWebRequest.Get(
                        "https://raw.githubusercontent.com/developer9998/MonkePhone/main/FreeCloudStorageGalore.json");
        ...
        if (versionWebRequest.result == UnityWebRequest.Result.Success)
        {
            ...
            if (Data != null && Version.TryParse(Constants.Version, out Version installed) &&
                Version.TryParse(Data.version,                      out Version version)   && version > installed)
            {
        ...
        {
            Logging.Error(
                    $"Error when checking version (maybe pastebin is gone?): {versionWebRequest.downloadHandler.text}");
...
#region Sounds
    ...
    public void PlaySound(string soundId, float volume = 1f)
    {
        ...
        AudioSource audio = _audioSourceCache.Count == 0
                                    ? null
                                    : _audioSourceCache.FirstOrDefault(audio => audio && !audio.isPlaying);

        if (!audio)
        {
            GameObject audioObject = new($"Audio Cache #{_audioSourceCache.Count + 1}", typeof(AudioSource));
...
#region Apps
    ...
    public T GetApp<T>() where T : PhoneApp, IPhoneApp
    {
        ...
        foreach (PhoneApp app in _storedApps)
            if (app.TryGetComponent(typeof(T), out Component component))
                return (T)component;
    ...
    public PhoneApp GetApp(string appId) =>
            !_storedApps.Any() ? null : _storedApps.FirstOrDefault(app => app.AppId == appId);
