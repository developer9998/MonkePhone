...
public class ConfigurationApp : PhoneApp
{
    ...
    public override void AppOpened()
    {
        ...
        if (_configurationEntries == null)
        {
            ...
            _configurationEntries = configFile.Keys
                                              .Where(definition =>
                                                             definition.Key != "Wallpaper" &&
                                                             !definition.Key.StartsWith("Upload"))
                                              .Select(definition => configFile[definition]);
        }
    ...
    private void RefreshApp()
    {
        ...
        if (isEnum)
        {
            ...
            DescriptionAttribute description = (DescriptionAttribute)entry.SettingType
                                                                          .GetMember(entry.GetSerializedValue())
                                                                         ?.First()?.GetCustomAttributes(
            ...
            _slider.transform.Find("Description").GetComponent<Text>().text =
                    description != null ? description.Description : string.Empty;

            slider.transform.localPosition = description != null
                                                     ? new Vector3(12.9f, -31.9f, -0.2000008f)
                                                     : new Vector3(12.9f, -26.5f, -0.2000008f);
    ...
    public override void SliderUpdated(PhoneUIObject phoneUIObject, float currentValue, bool isSelected)
    {
        ...
        DescriptionAttribute description = (DescriptionAttribute)entry.SettingType.GetMember(currentEnum)?.First()
        ...
        phoneUIObject.transform.localPosition = description != null
                                                        ? new Vector3(12.9f, -31.9f, -0.2000008f)
                                                        : new Vector3(12.9f, -26.5f, -0.2000008f);
    }
}