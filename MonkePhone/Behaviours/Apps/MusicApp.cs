...
public class MusicApp : PhoneApp
{
    ...
    private readonly Dictionary<string, AudioClip> _musicComparison = [];

    private readonly List<StreamableMusicComponent> _streambleMusicComponents = [];
    private readonly bool                           _wasPlaying               = true;
    ...
    public override void Initialize()
    {
        ...
        _songTimePosition = transform.Find("CurrentlyPlayingContents/Timeline/Slider/Text (Legacy)")
                                     .GetComponent<Text>();
    ...
    {
        List<string> current = _musicList;

        _musicList = Directory.GetFiles(PhoneManager.Instance.MusicPath)
                              .Where(file => file.ToLower().EndsWith(".mp3") || file.ToLower().EndsWith(".ogg") ||
                                             file.ToLower().EndsWith(".wav")).ToList();

        IEnumerable<string> missingItems =
                current.Where(str => !_musicList.Contains(str) && _musicComparison.ContainsKey(str));

        foreach (string str in missingItems)
            _musicComparison.Remove(str);
    }

    /// <summary>
    ///     Refreshes the current / suitable container, if we are viewing the download view, the downloadable container will be
    ///     refreshed, vice versa.
    /// </summary>
    ...
    private void RefreshDownloadables()
    {
        ...
        for (int i = 0; i < table.childCount; i++)
        {
            ...
            bool noSong = song == null || song == default(object);
    ...
    private void RefreshSongList()
    {
        ...
        for (int i = 0; i < table.childCount; i++)
        {
            ...
            bool noSong = song == null || song == default(string);
            item.gameObject.SetActive(!noSong);
            ...
            async void HandleButton()
            {
                ...
                item.Find("Format Label").GetComponent<Text>().text =
                        Path.GetExtension(song).Replace(".", "").ToUpper();

                item.Find("Length Label").GetComponent<Text>().text =
                        TimeSpan.FromSeconds(_musicComparison[song].length).ToString(@"mm\:ss");
            }
    ...
    public override async void ButtonClick(PhoneUIObject phoneUIObject, bool isLeftHand)
    {
        if (phoneUIObject.name.StartsWith("download"))
        {
            ...
            StreamableMusicComponent component = _streambleMusicComponents.ElementAtOrDefault(index);

            if (!component) return;

            component.UpdateText(component.song.currentState != Song.DownloadState.Downloaded
                                         ? Song.DownloadState.Awaiting
                                         : Song.DownloadState.Downloaded);

            Song.DownloadState state = await component.song.Download();
    ...
    public async Task LoadTrack(string path)
    {
        ...
        DownloadHandlerAudioClip downloadHandler = new(path, audioType);
        UnityWebRequest          webRequest      = new(path, "GET", downloadHandler, null);
