...
public class MonkeGramApp : PhoneApp
{
    ...
    private         float  shutter_elapsed_time;
    ...
    public void Update()
    {
        Camera.nearClipPlane = Constants.NearClipPlane * GTPlayer.Instance.scale;
        Camera.farClipPlane  = Camera.main.farClipPlane;

        float value = Phone.LeftHand
                              ? ControllerInputPoller.instance.leftControllerIndexFloat
        ...
        shutter_elapsed_time =
                Mathf.Min(Mathf.Approximately(shutter_elapsed_time, 0) ? 0 : shutter_elapsed_time + Time.deltaTime, 1);
    ...
    public void FlipCamera(bool flip)
    {
        ...
        Camera.transform.localPosition =
                _isCameraFlipped ? Constants.CameraBackward.Position : Constants.CameraForward.Position;

        Camera.transform.localRotation =
                _isCameraFlipped ? Constants.CameraBackward.Rotation : Constants.CameraForward.Rotation;
    }
    ...
    public async void Shutter()
    {
        ...
        string currentName =
                PhotonNetwork.LocalPlayer.GetName(GorillaTagger.Instance.offlineVRRig,
                        false); // we already know our player has the phone, they're taking the photo
        ...
        if (ModIOManager.IsLoggedIn())
        {
            ...
            {
                (Error error, Mod mod) = await ModIOManager.GetMod(currentRoomMap);
        ...
        if (Configuration.PlayerMention.Value && PhotonNetwork.InRoom && PhotonNetwork.CurrentRoom.PlayerCount > 1)
        {
            ...
            IEnumerable<VRRig> supportiveRigs =
                    GorillaParent.instance.vrrigs.Where(rig => rig.Creator != null && !rig.Creator.IsLocal &&
                                                               Camera.PointInCameraView(rig.tagSound.transform.position,
                                                                       true, UnityLayer.GorillaTagCollider,
                                                                       supportiveLayerMask));

            bool localInView = Camera.PointInCameraView(GTPlayer.Instance.headCollider.transform.position);

            _finalContents += supportiveRigs.Any()
                                      ? $"{(localInView ? "with" : "of")} {supportiveRigs.Select(rig => rig.Creator.GetName(rig)).ListElements()}{(Configuration.RevealMap.Value ? $" in {currentMap}" : "")}"
                                      : Configuration.RevealMap.Value
                                              ? $"in {currentMap}"
                                              : "";
        }
        else
        ...
        if (_finalTexture is not null)
        {
            ...
            string path = Path.Combine(PhoneManager.Instance.PhotosPath, name);
