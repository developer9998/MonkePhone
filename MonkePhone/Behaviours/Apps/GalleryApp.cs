...
namespace MonkePhone.Behaviours.Apps
{
    public class GalleryApp : PhoneApp
    {
        ...
        public override void AppOpened()
        {
            ...
            _photoComparison = Directory.GetFiles(PhoneManager.Instance.PhotosPath).Where(file => file.EndsWith(".png") || file.EndsWith(".jpg") || file.EndsWith(".jpeg") || file.EndsWith(".gif")).ToDictionary(photo => photo, photo => Path.GetFileName((photo)));
        ...
        public override void ButtonClick(PhoneUIObject phoneUIObject, bool isLeftHand)
        {
            ...
            switch (phoneUIObject.name)
            {
                ...
                case "Photos Trash":
                    ...
                    string path = Path.Combine(PhoneManager.Instance.PhotosPath, _photoComparison.ElementAt(_currentPhoto).Value);

                    try
                    {
                        if (!FileEx.RecycleFile(path)) File.Delete(path);
                    }
                    catch
                    {
                        File.Delete(path);
                ...
                case "SetThumbnail":
                    ...
                    fileName = _photoComparison.ElementAt(_currentPhoto).Value;
                    if (Configuration.WallpaperName.Value == fileName) PhoneManager.Instance.ApplyWallpaper(true, string.Empty);
        ...
        public async void SendWebhook(string message, string image_name, byte[] image)
        {
            ...
            switch (Configuration.UploadMethod.Value)
            {
                ...
                case Configuration.EUploadMethod.CustomServer:
                    ...
                    {
                        Logging.Error($"A photo was unable to be uploaded to the website: {webRequest.error} ({webRequest.responseCode})");
                        Logging.Warning($"Server response: {webRequest.downloadHandler.text}");
                        ...
                        if (!isRelevant) return;
                        _uploadButton.transform.Find("Image").GetComponent<Image>().color = Color.red;
                        _gallerySelection.text = $"Error {webRequest.responseCode}{((!string.IsNullOrEmpty(webRequest.error) && !string.IsNullOrWhiteSpace(webRequest.error)) ? $": {webRequest.error}" : string.Empty)}";
                    }
                    break;
            }
        }
